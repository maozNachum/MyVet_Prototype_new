create or replace function private.myvet_prepare_owner_signup_context()
returns trigger language plpgsql security definer set search_path = '' as $$
begin
  if lower(btrim(coalesce(new.raw_user_meta_data ->> 'role','')))='owner' and new.email_confirmed_at is not null then perform pg_catalog.set_config('myvet.owner_signup_user_id',new.id::text,true); end if;
  return new;
end;
$$;
revoke all on function private.myvet_prepare_owner_signup_context() from public,anon,authenticated;
drop trigger if exists a_myvet_prepare_owner_signup_context on auth.users;
create trigger a_myvet_prepare_owner_signup_context before insert or update on auth.users for each row execute function private.myvet_prepare_owner_signup_context();

create or replace function private.myvet_enforce_tenant_write()
returns trigger language plpgsql security definer set search_path = ''
as $$
declare
  old_data jsonb:=case when tg_op='INSERT' then '{}'::jsonb else to_jsonb(old) end; new_data jsonb:=case when tg_op='DELETE' then '{}'::jsonb else to_jsonb(new) end;
  target_clinic_id uuid:=coalesce(nullif(new_data->>'clinic_id','')::uuid,nullif(old_data->>'clinic_id','')::uuid);
  jwt_role text:=coalesce(nullif(current_setting('request.jwt.claim.role',true),''),nullif(current_setting('request.jwt.claims',true),'')::jsonb->>'role',''); trusted_database_session boolean:=pg_has_role(session_user,'postgres','member');
...
begin
  if target_clinic_id is null then raise exception 'TENANT_REQUIRED'; end if; if tg_op='UPDATE' and nullif(old_data->>'clinic_id','')::uuid is distinct from nullif(new_data->>'clinic_id','')::uuid then raise exception 'TENANT_CHANGE_FORBIDDEN'; end if;
  if trusted_owner_signup_context then if tg_op='DELETE' then return old; end if; return new; end if;
  ...
  if not private.myvet_user_has_clinic_access(target_clinic_id) then raise exception 'TENANT_ACCESS_DENIED'; end if; if tg_op='DELETE' then return old; end if; return new;
end;
$$;
revoke all on function private.myvet_enforce_tenant_write() from public,anon,authenticated,service_role;

-- Rollback: drop a_myvet_prepare_owner_signup_context and restore the guard from 20260915170000.
