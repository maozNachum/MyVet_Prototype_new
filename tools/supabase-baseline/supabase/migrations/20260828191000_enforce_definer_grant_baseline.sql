-- Clean-room normalization: reconstruct the verified role boundary explicitly.
-- The temporary pg_dump contained stale authenticated grants and PostgreSQL also
-- grants PUBLIC EXECUTE by default. Start closed, then restore the allowlisted
-- Production roles plus the roles declared by the pending migrations.
do $$
declare
  function_row record;
  expected_name text;
...
begin
  ...
  loop
    if not exists (
      ...
      where n.nspname = 'public' and p.proname = expected_name and p.prosecdef
    ) then
  ...
  loop
    if not exists (
      select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
      where n.nspname = 'private' and p.proname = expected_name and p.prosecdef
    ) then
  ...
  for function_row in
    ...
    where n.nspname = 'public' and p.proname = any(auth_and_service)
  loop
    execute format('grant execute on function %s to authenticated, service_role', function_row.function_identity);
  end loop;
  ...
  loop
    execute format('grant execute on function %s to authenticated', function_row.function_identity);
  end loop;
  ...
  loop
    execute format('grant execute on function %s to service_role', function_row.function_identity);
  end loop;

  for function_row in
    select p.oid::regprocedure as function_identity
    from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'private' and p.proname = any(private_auth_and_service)
  loop
    execute format('grant execute on function %s to authenticated, service_role', function_row.function_identity);
  end loop;
end;
