-- Fail closed when a verified Auth email matches more than one owner profile.
-- The browser supplies no owner or clinic identifier; identity is derived only
...
begin
  ...
  -- A JWT email is accepted only when it still matches the confirmed Auth
  -- identity. User metadata and browser input are never authorization sources.
  ...
  -- Retrying a completed claim is idempotent, including after a transient
  -- client failure. The unique auth_user_id constraint guarantees one result.
  ...
  -- Lock every matching owner row. This prevents two concurrent callers from
  -- observing and claiming the same unlinked profile at the same time.
  ...
  if matching_owner_count = 0 then
    return null;
  end if;
  ...
  if candidate_auth_user_id is not null then
    if candidate_auth_user_id = actor_id then
...
comment on function public.claim_owner_profile() is
  'Atomically claims exactly one owner row using the confirmed Supabase Auth email; ambiguous matches fail closed and no tenant identifier is accepted from the browser.';
