-- Permit exactly one initial clinic_admin membership while a new clinic is
-- created by the authenticated onboarding RPC. Direct browser table writes
-- remain blocked by grants, RLS, and the tenant write guard.
...
declare
  ...
  trusted_auth_owner_signup boolean := session_user = 'supabase_auth_admin' and current_user = 'postgres'
    and tg_table_schema = 'public' and tg_table_name = 'owners' and tg_op in ('INSERT', 'UPDATE') and pg_trigger_depth() > 1;
  trusted_clinic_admin_bootstrap boolean := tg_table_schema = 'public' and tg_table_name = 'staff' and tg_op = 'INSERT'
    and current_setting('myvet.bootstrap_clinic_id', true) = target_clinic_id::text
    and current_setting('myvet.bootstrap_actor_id', true) = coalesce(new_data ->> 'auth_user_id', '')
    and nullif(new_data ->> 'auth_user_id', '')::uuid = (select auth.uid())
    and new_data ->> 'role' = 'clinic_admin'
    and not exists (select 1 from public.staff as existing_staff where existing_staff.clinic_id = target_clinic_id);
begin
  if target_clinic_id is null then raise exception 'TENANT_REQUIRED'; end if;
  if tg_op = 'UPDATE' and nullif(old_data ->> 'clinic_id', '')::uuid is distinct from nullif(new_data ->> 'clinic_id', '')::uuid then
    raise exception 'TENANT_CHANGE_FORBIDDEN';
  end if;
  if (select auth.uid()) is null then
    if jwt_role = 'service_role' or trusted_database_session or trusted_auth_owner_signup then
      if tg_op = 'DELETE' then return old; end if;
      return new;
    end if;
    raise exception 'AUTH_REQUIRED';
  end if;
  if trusted_clinic_admin_bootstrap then return new; end if;
  if tg_table_schema = 'public' and tg_table_name = 'owners' and tg_op = 'UPDATE'
    and nullif(old_data ->> 'auth_user_id', '') is null
    and nullif(new_data ->> 'auth_user_id', '')::uuid = (select auth.uid())
    and nullif(old_data ->> 'clinic_id', '')::uuid = target_clinic_id then return new;
  end if;
  if tg_table_schema = 'public' and tg_table_name = 'owners' and tg_op = 'INSERT'
    and nullif(new_data ->> 'auth_user_id', '')::uuid = (select auth.uid())
    and target_clinic_id = private.myvet_current_clinic_id() then return new;
  end if;
...
begin
  ...
  if exists (select 1 from public.staff as staff_member where staff_member.auth_user_id = actor_user_id and staff_member.is_active = true and staff_member.role <> 'clinic_admin')
    or exists (select 1 from public.owners as owner where owner.auth_user_id = actor_user_id) then raise exception 'CLINIC_ADMIN_REQUIRED';
  end if;
  select auth_user.email, coalesce(nullif(btrim(auth_user.raw_user_meta_data ->> 'full_name'), ''), auth_user.email)
  into actor_email, actor_name from auth.users as auth_user where auth_user.id = actor_user_id;
  insert into public.clinics(slug, display_name) values (normalized_slug, normalized_name)
...
-- Rollback: restore both function definitions from migration
-- 20260915120000_multi_clinic_onboarding_and_secure_owner_signup.sql.
