-- Baseline copy is intentionally identical to the root migration's behavior.
...
declare
  ...
  trusted_staff_invitation boolean := tg_table_schema = 'public' and tg_table_name = 'staff' and tg_op in ('INSERT', 'UPDATE') and current_setting('myvet.invitation_actor_id', true) = (select auth.uid())::text and nullif(new_data ->> 'auth_user_id', '')::uuid = (select auth.uid()) and exists (select 1 from public.clinic_invitations as invitation join auth.users as auth_user on auth_user.id = (select auth.uid()) where invitation.invitation_id::text = current_setting('myvet.invitation_id', true) and invitation.clinic_id = target_clinic_id and invitation.invitation_type = 'staff' and invitation.role = new_data ->> 'role' and lower(btrim(invitation.email)) = lower(btrim(auth_user.email)) and auth_user.email_confirmed_at is not null and invitation.accepted_at is null and invitation.revoked_at is null and invitation.expires_at > now());
begin
...
create or replace function public.myvet_accept_clinic_invitation(requested_token text, requested_full_name text default null, requested_phone text default null, requested_owner_id text default null, requested_terms_version text default null)
returns jsonb language plpgsql security definer set search_path = ''
as $$
declare
  actor_user_id uuid := (select auth.uid()); actor_email text; invite public.clinic_invitations%rowtype; target_owner_id text; target_staff_id uuid;
  normalized_name text := btrim(coalesce(requested_full_name, '')); normalized_phone text := regexp_replace(coalesce(requested_phone, ''), '[^0-9]', '', 'g');
begin
  ...
  select invitation.* into invite from public.clinic_invitations as invitation where invitation.token_hash = encode(sha256(convert_to(btrim(requested_token), 'UTF8')), 'hex') and invitation.accepted_at is null and invitation.revoked_at is null and invitation.expires_at > now() and lower(btrim(invitation.email)) = actor_email for update;
  if not found then raise exception 'INVITATION_INVALID_OR_EXPIRED'; end if;
  if invite.invitation_type = 'owner' then
    target_owner_id := coalesce(nullif(btrim(requested_owner_id), ''), invite.owner_id); if target_owner_id <> invite.owner_id then raise exception 'INVITATION_OWNER_MISMATCH'; end if;
    if normalized_name = '' or normalized_phone !~ '^05[0-9]{8}$' then raise exception 'OWNER_SIGNUP_INVALID_PROFILE'; end if; if requested_terms_version <> 'myvet-owner-portal-v1' then raise exception 'OWNER_SIGNUP_TERMS_REQUIRED'; end if;
    update public.owners set auth_user_id=actor_user_id,email=actor_email,owner_first_name=split_part(normalized_name,' ',1),owner_last_name=btrim(substr(normalized_name,char_length(split_part(normalized_name,' ',1))+1)),phone=normalized_phone,terms_accepted_at=now(),terms_version=requested_terms_version where clinic_id=invite.clinic_id and owner_id=target_owner_id and auth_user_id is null;
    if not found then insert into public.owners(clinic_id,owner_id,auth_user_id,owner_first_name,owner_last_name,phone,email,terms_accepted_at,terms_version) values(invite.clinic_id,target_owner_id,actor_user_id,split_part(normalized_name,' ',1),btrim(substr(normalized_name,char_length(split_part(normalized_name,' ',1))+1)),normalized_phone,actor_email,now(),requested_terms_version); end if;
    update public.clinic_invitations set accepted_at=now(),updated_at=now() where invitation_id=invite.invitation_id; return jsonb_build_object('kind','owner','clinic_id',invite.clinic_id,'owner_id',target_owner_id);
  end if;
  if normalized_name='' then normalized_name:=split_part(actor_email,'@',1); end if;
  perform pg_catalog.set_config('myvet.invitation_id',invite.invitation_id::text,true); perform pg_catalog.set_config('myvet.invitation_actor_id',actor_user_id::text,true);
  select staff_member.staff_id into target_staff_id from public.staff as staff_member where staff_member.clinic_id=invite.clinic_id and staff_member.auth_user_id=actor_user_id order by staff_member.created_at desc limit 1;
  if target_staff_id is null then insert into public.staff(clinic_id,auth_user_id,email,full_name,name,role,is_active) values(invite.clinic_id,actor_user_id,actor_email,normalized_name,normalized_name,invite.role,true) returning staff_id into target_staff_id;
  else update public.staff set email=actor_email,full_name=normalized_name,name=normalized_name,role=invite.role,is_active=true where staff_id=target_staff_id; end if;
  update public.clinic_invitations set accepted_at=now(),updated_at=now() where invitation_id=invite.invitation_id;
  perform pg_catalog.set_config('myvet.invitation_id','',true); perform pg_catalog.set_config('myvet.invitation_actor_id','',true);
