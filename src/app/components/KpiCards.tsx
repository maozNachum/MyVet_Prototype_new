...
// ─── Data Arrays ────────────────────────────────────────────────────────
const departments = [
  { id: 1, name: "מחלקה פנימית", icon: "🏥", color: "bg-blue-50 border-blue-200 text-blue-700", patients: [ { id: 1, name: "רקס", species: "כלב", owner: "יוסי כהן", status: "מאושפז - יום 2", statusColor: "bg-blue-100 text-blue-700" }, { id: 2, name: "לונה", species: "כלב", owner: "מיכל לוי", status: "מאושפזת - יום 1", statusColor: "bg-blue-100 text-blue-700" } ] },
  { id: 2, name: "מחלקה כירורגית", icon: "🔬", color: "bg-purple-50 border-purple-200 text-purple-700", patients: [ { id: 3, name: "מקס", species: "כלב", owner: "דני אברהם", status: "לאחר ניתוח", statusColor: "bg-amber-100 text-amber-700" } ] },
  { id: 3, name: "טיפול נמרץ", icon: "🚨", color: "bg-red-50 border-red-200 text-red-700", patients: [ { id: 4, name: "באדי", species: "כלב", owner: "רונית שמש", status: "מצב יציב", statusColor: "bg-green-100 text-green-700" } ] },
  { id: 4, name: "אשפוז יום", icon: "☀️", color: "bg-amber-50 border-amber-200 text-amber-700", patients: [ { id: 5, name: "ניקו", species: "חתול", owner: "שרה לוי", status: "ממתין לשחרור", statusColor: "bg-emerald-100 text-emerald-700" }, { id: 6, name: "מיאו", species: "חתול", owner: "שרה גולדברג", status: "בהשגחה", statusColor: "bg-blue-100 text-blue-700" } ] },
];

const urgentCases = [
  { id: 1, petName: "מקס", species: "כלב", owner: "דני אברהם", issue: "ניתוח חירום - גוף זר במע' העיכול", severity: "קריטי", sevColor: "bg-red-100 text-red-700", time: "הגיע ב-08:15" },
  { id: 2, petName: "באדי", species: "כלב", owner: "רונית שמש", issue: "הרעלת שוקולד - ניטור", severity: "בינוני", sevColor: "bg-orange-100 text-orange-700", time: "הגיע ב-07:40" },
  { id: 3, petName: "שוקו", species: "ארנב", owner: "נועה פרץ", issue: "שבר ברגל אחורית", severity: "דחוף", sevColor: "bg-amber-100 text-amber-700", time: "הגיע ב-09:00" },
];
        ...
        <div className="bg-white rounded-2xl shadow-md border border-emerald-200 overflow-hidden animate-in fade-in slide-in-from-top-2">
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            {departments.map((dept) => (
              <div key={dept.id} className={`rounded-xl border p-4 ${dept.color}`}>
        ...
        <div className="bg-white rounded-2xl shadow-md border border-orange-200 overflow-hidden animate-in fade-in slide-in-from-top-2">
          ...
          <div className="p-4 space-y-3">
            {urgentCases.map((uc) => {
              ...
              return (
                <div key={uc.id} className="flex items-center gap-4 p-4 rounded-xl border border-gray-100 hover:border-orange-200 hover:bg-orange-50/20 transition-all">
                  <div className="w-11 h-11 bg-orange-50 rounded-xl flex items-center justify-center shrink-0"><UCIcon className="w-5 h-5 text-orange-500" /></div>
                  <div className="flex-1 min-w-0">
