import { Clock, X, Dog, Cat, Calendar, Building2, Stethoscope, AlertTriangle, Activity, Pill, ClipboardList, Loader2 } from "lucide-react";
...
function appointmentStartDate(appt: CalendarAppointment) {
  ...
  return new Date(appt.year, appt.month, appt.day, hour || 0, minute || 0, 0, 0);
}
...
function getStatusUI(appt: CalendarAppointment) {
  ...
  if (Number.isNaN(start.getTime())) {
    return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-gray-50 text-gray-600 border border-gray-200 text-[12px] font-semibold"><span className="w-1.5 h-1.5 rounded-full bg-gray-400" /> מתוכנן</span>;
  }

  if (now < start) {
    return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-gray-50 text-gray-600 border border-gray-200 text-[12px] font-semibold"><span className="w-1.5 h-1.5 rounded-full bg-gray-400" /> טרם הגיע</span>;
  }

  if (now >= start && now <= end) {
    return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 border border-blue-100 text-[12px] font-semibold"><span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" /> עכשיו</span>;
  }

  return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-100 text-[12px] font-semibold"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> עבר</span>;
}
...
function visitTypeLabel(type?: string | null) {
  switch (type) {
    case "full_exam": return "בדיקה רפואית מלאה";
    case "vaccination": return "חיסון";
    case "weight_check": return "שקילה";
    case "prescription_only": return "מרשם בלבד";
    case "lab": return "בדיקת מעבדה";
    case "follow_up": return "מעקב קצר";
    case "note": return "הערה רפואית";
    default: return type || "ביקור רפואי";
  }
}
...
export function AppointmentsTable() {
  ...
  const [treatmentAppt, setTreatmentAppt] = useState<CalendarAppointment | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadSnapshot() {
      ...
      try {
        const [{ data: patientRows, error: patientError }, { data: visitsRows, error: visitsError }, { data: prescriptionRows, error: prescriptionsError }, { data: problemRows, error: problemsError }] = await Promise.all([
          supabase
            .from("patients")
            .select("pet_id, pet_name, species, breed, weight, allergies, owner_id")
            .eq("pet_id", selectedAppt.petId)
            .limit(1),
          supabase
            .from("medical_visits")
            .select("visit_id, visit_date, visit_type, chief_complaint, final_diagnosis, treatment, notes")
            .eq("pet_id", selectedAppt.petId)
            .order("visit_date", { ascending: false })
            .limit(3),
          supabase
            .from("prescriptions")
            .select("prescription_id, medication, dosage, frequency, duration, start_date")
            .eq("pet_id", selectedAppt.petId)
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end">
                          {canTreat && (
                            ...
                            >
                              <Stethoscope className="w-3.5 h-3.5 opacity-80" /> הוסף רשומה
                            </button>
                          )}
                        </div>
                      </td>
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-10 text-center text-gray-500 text-[14px]">
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="bg-gradient-to-l from-[#1e40af] to-[#2563eb] px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
                  {selectedAppt.petSpecies === "cat" ? <Cat className="w-6 h-6 text-white" /> : <Dog className="w-6 h-6 text-white" />}
                </div>
                <div>
                  <h3 className="text-white text-[20px]" style={{ fontWeight: 700 }}>{selectedAppt.petName}</h3>
                  <p className="text-white/80 text-[13px]">{selectedAppt.ownerName} | {selectedAppt.type} | {selectedAppt.time}</p>
                </div>
              </div>
              <button type="button" onClick={() => setSelectedAppt(null)} className="text-white/60 hover:text-white cursor-pointer p-1"><X className="w-6 h-6" /></button>
            </div>
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              {isSnapshotLoading ? (
                ...
                <div className="md:col-span-2 py-8 text-center text-red-500 text-[14px]">{snapshotError}</div>
              ) : (
                <>
                    ...
                    <div>
                      ...
                      {snapshot?.patient?.allergies ? (
                        <div className="flex flex-wrap gap-2">
                          {snapshot.patient.allergies.split(/[,;\n]/).map((a) => a.trim()).filter(Boolean).map((allergy) => (
                            <span key={allergy} className="bg-red-50 text-red-700 border border-red-100 px-2.5 py-1 rounded-lg text-[12px] font-semibold">{allergy}</span>
                          ))}
                        </div>
                      ) : (
                        <p className="text-gray-500 text-[13px] bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">לא תועדו רגישויות במסד.</p>
                      )}
                    </div>

                    <div>
                      <h4 className="flex items-center gap-2 text-gray-800 text-[14px] mb-2" style={{ fontWeight: 700 }}>
                        <ClipboardList className="w-4 h-4 text-blue-500" /> בעיות רפואיות פעילות
                      </h4>
                      {snapshot?.activeProblems?.length ? (
                        <ul className="space-y-1.5">
                          {snapshot.activeProblems.map((problem) => (
                            <li key={problem.problem_id} className="text-gray-600 text-[13px] bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100 flex items-center justify-between gap-2">
                              <span>{problem.problem_text || "בעיה רפואית"}</span>
                              <span className="text-[11px] text-orange-600 font-semibold">{severityLabel(problem.severity)}</span>
                            </li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-gray-500 text-[13px] bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">אין בעיות פעילות מתועדות.</p>
                      )}
                    </div>

                    <div>
                      <h4 className="flex items-center gap-2 text-gray-800 text-[14px] mb-2" style={{ fontWeight: 700 }}>
                        <Pill className="w-4 h-4 text-indigo-500" /> מרשמים אחרונים
                      </h4>
                      {snapshot?.prescriptions?.length ? (
                        <ul className="space-y-1.5">
                          {snapshot.prescriptions.map((rx) => (
                            <li key={rx.prescription_id} className="text-gray-600 text-[13px] bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                              <span className="font-semibold text-gray-800">{rx.medication || "תרופה"}</span>
                              {(rx.dosage || rx.frequency || rx.duration) && <span> — {[rx.dosage, rx.frequency, rx.duration].filter(Boolean).join(" · ")}</span>}
                            </li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-gray-500 text-[13px] bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">אין מרשמים אחרונים במסד.</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-5">
                    <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-4">
                      ...
                      <div className="space-y-3">
                        ...
                        {selectedAppt.notes && <div><p className="text-gray-500 text-[11px] font-semibold">הערות תור</p><p className="text-gray-900 text-[13px]">{selectedAppt.notes}</p></div>}
                      </div>
                    </div>
                    ...
                    <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl p-4">
                      <div className="flex justify-between items-center mb-3">
                        <h4 className="flex items-center gap-2 text-emerald-800 text-[14px]" style={{ fontWeight: 700 }}>
                          <Activity className="w-4 h-4 text-emerald-500" /> נתונים רפואיים אחרונים
                        </h4>
                      </div>
                      <div className="space-y-3">
                        <div className="flex gap-4">
                          <div>
                            <p className="text-gray-500 text-[11px] font-semibold">משקל</p>
                            <p className="text-gray-900 text-[15px] font-bold">{snapshot?.patient?.weight ? `${snapshot.patient.weight} ק״ג` : "לא תועד"}</p>
                          </div>
                          <div>
                            <p className="text-gray-500 text-[11px] font-semibold">ביקור אחרון</p>
                            <p className="text-gray-900 text-[15px] font-bold">{snapshot?.visits?.[0]?.visit_date ? formatDate(snapshot.visits[0].visit_date) : "אין"}</p>
                          </div>
                        </div>
                        {snapshot?.visits?.length ? (
                          <div className="space-y-1.5 pt-2 border-t border-emerald-100">
                            {snapshot.visits.map((visit) => (
                              <div key={visit.visit_id} className="text-[12px] text-gray-600 bg-white/70 rounded-lg px-3 py-2 border border-emerald-100">
                                <span className="font-semibold text-gray-800">{visitTypeLabel(visit.visit_type)}</span>
                                {visit.final_diagnosis && <span> · {visit.final_diagnosis}</span>}
                                {visit.chief_complaint && <span> · {visit.chief_complaint}</span>}
                              </div>
                            ))}
                          </div>
                        ) : null}
            ...
            <div className="bg-gray-50 border-t border-gray-100 px-6 py-4 flex gap-3">
              ...
              <button type="button" onClick={() => setSelectedAppt(null)} className="px-6 py-3 border border-gray-200 bg-white rounded-xl text-gray-600 hover:bg-gray-50 transition-colors text-[14px] font-bold cursor-pointer">
                סגור תצוגה
              </button>
            </div>
          </div>
