...
async function database() {
  ...
  await db.exec(`
    ...
    grant execute on function auth.uid(),public.myvet_current_owner_id(),private.myvet_current_clinic_id(),public.myvet_is_active_staff() to authenticated,service_role;
...
async function asUser(db, userId, role = "authenticated") {
  await db.exec("reset role");
  await db.query("select set_config('request.jwt.claim.sub',$1,false)", [userId || ""]);
  await db.exec(`set role ${role}`);
}
async function reset(db) { await db.exec("reset role"); await db.query("select set_config('request.jwt.claim.sub','',false)"); }
...
test("only an active clinic admin can manage a request in the same clinic", async () => {
  ...
  try {
    ...
    await asUser(db,id.vetA); await assert.rejects(db.query("select public.myvet_manage_privacy_request($1,'in_review',null)",[requestId]),/CLINIC_ADMIN_REQUIRED/);
    await asUser(db,id.adminB); await assert.rejects(db.query("select public.myvet_manage_privacy_request($1,'in_review',null)",[requestId]),/PRIVACY_REQUEST_NOT_FOUND/);
    await asUser(db,id.adminA); await db.query("select public.myvet_manage_privacy_request($1,'completed','reviewed')",[requestId]);
    await reset(db);
