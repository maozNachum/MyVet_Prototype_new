-- Complete the narrow clinic bootstrap context before the clinics INSERT so
-- the existing AFTER INSERT seed trigger can create disabled AI feature flags.
...
declare
  ...
  bootstrap_context_matches boolean :=
    current_setting('myvet.bootstrap_clinic_id', true) = target_clinic_id::text
    and current_setting('myvet.bootstrap_actor_id', true) = (select auth.uid())::text;
  trusted_clinic_admin_bootstrap boolean := bootstrap_context_matches
    and tg_table_schema = 'public' and tg_table_name = 'staff' and tg_op = 'INSERT'
  ...
  trusted_ai_flag_bootstrap boolean := bootstrap_context_matches
    and tg_table_schema = 'public' and tg_table_name = 'ai_feature_flags' and tg_op = 'INSERT'
    and coalesce((new_data ->> 'enabled')::boolean, false) = false
    and not exists (select 1 from public.staff as existing_staff where existing_staff.clinic_id = target_clinic_id);
begin
...
begin
  ...
  if exists (select 1 from public.staff as staff_member where staff_member.auth_user_id = actor_user_id and staff_member.is_active = true and staff_member.role <> 'clinic_admin')
    or exists (select 1 from public.owners as owner where owner.auth_user_id = actor_user_id) then raise exception 'CLINIC_ADMIN_REQUIRED'; end if;
