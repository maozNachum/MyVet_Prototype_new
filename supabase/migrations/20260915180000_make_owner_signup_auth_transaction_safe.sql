-- Avoid recursively updating auth.users from its own AFTER trigger. The same
-- validated owner provisioning now runs BEFORE the Auth write and sanitizes NEW.
...
begin
  ...
  select owner.* into existing_owner from public.owners as owner where owner.owner_id = requested_owner_id for update;
  ...
  if requested_invitation_token <> '' then
    ...
    where invitation.token_hash = encode(sha256(convert_to(requested_invitation_token, 'UTF8')), 'hex')
      and invitation.invitation_type = 'owner' and invitation.owner_id = requested_owner_id
      and lower(btrim(invitation.email)) = requested_email and invitation.accepted_at is null
      and invitation.revoked_at is null and invitation.expires_at > now() for update;
  ...
  if existing_owner_found then
    ...
    update public.owners set auth_user_id = new.id, owner_first_name = requested_first_name,
    ...
    values (target_clinic_id, requested_owner_id, new.id, requested_first_name, requested_last_name, requested_phone, requested_email, now(), requested_terms_version);
    update public.clinic_invitations set accepted_at = now(), updated_at = now() where invitation_id = invite.invitation_id;
  end if;
...
create trigger on_auth_user_created_myvet_owner before insert on auth.users
for each row execute function private.myvet_handle_owner_signup();
create trigger on_auth_user_confirmed_myvet_owner before update of email_confirmed_at on auth.users
for each row when (old.email_confirmed_at is null and new.email_confirmed_at is not null)
execute function private.myvet_handle_owner_signup();
create trigger on_auth_user_metadata_myvet_owner before update of raw_user_meta_data on auth.users
for each row when (old.raw_user_meta_data is distinct from new.raw_user_meta_data and lower(btrim(coalesce(new.raw_user_meta_data ->> 'role', ''))) = 'owner')
execute function private.myvet_handle_owner_signup();
