...
as $$
begin
  if lower(btrim(coalesce(new.raw_user_meta_data ->> 'role', ''))) = 'owner'
    and new.email_confirmed_at is not null then
    perform pg_catalog.set_config('myvet.owner_signup_user_id', new.id::text, true);
  end if;
  return new;
end;
$$;
...
create trigger a_myvet_prepare_owner_signup_context before insert or update on auth.users
for each row execute function private.myvet_prepare_owner_signup_context();
