...
import { buildClientSummaryUserPayload, buildDigitalCareSummaryUserPayload, buildFollowUpSuggestionUserPayload, buildRagAnswerUserPayload, buildVetBotUserPayload, buildVisitSummaryUserPayload, PROMPT_REGISTRY } from "./prompts.ts";
...
import { assertClientSummaryGrounded, CLIENT_SUMMARY_RESPONSE_SCHEMA, CLIENT_SUMMARY_SCHEMA_VERSION, DIGITALCARE_TRANSCRIPT_RESPONSE_SCHEMA, DIGITALCARE_TRANSCRIPT_SCHEMA_VERSION, DOCUMENT_EXTRACTION_RESPONSE_SCHEMA, DOCUMENT_EXTRACTION_SCHEMA_VERSION, FOLLOW_UP_SUGGESTION_RESPONSE_SCHEMA, FOLLOW_UP_SUGGESTION_SCHEMA_VERSION, RAG_ANSWER_RESPONSE_SCHEMA, RAG_ANSWER_SCHEMA_VERSION, validateClientSummaryOutput, validateDigitalCareTranscript, validateDocumentExtraction, validateFollowUpSuggestionOutput, validateRagAnswer, validateVetBotOutput, validateVisitSummaryOutput, VETBOT_OUTPUT_SCHEMA_VERSION, VETBOT_RESPONSE_SCHEMA, VISIT_SUMMARY_OUTPUT_SCHEMA_VERSION, VISIT_SUMMARY_RESPONSE_SCHEMA, type ValidatedClientSummaryOutput, type ValidatedDigitalCareTranscript, type ValidatedDocumentExtraction, type ValidatedFollowUpSuggestion, type ValidatedRagAnswer, type ValidatedVetBotOutput, type ValidatedVisitSummaryOutput } from "./schemas.ts";
import type { AiProviderAdapter, ClientSummaryGatewayInput, DigitalCareSummaryGatewayInput, DigitalCareTranscriptionGatewayInput, DocumentExtractionGatewayInput, DocumentExtractionProviderAdapter, EmbeddingProviderAdapter, EnvReader, FollowUpSuggestionGatewayInput, GatewayTelemetry, RagAnswerGatewayInput, TranscriptionProviderAdapter, VisitSummaryGatewayInput, VetBotGatewayInput, VetBotGatewayResult } from "./types.ts";
...
export async function runFollowUpSuggestionGateway(
  input: FollowUpSuggestionGatewayInput,
  options: VetBotGatewayOptions = {},
): Promise<VetBotGatewayResult<ValidatedFollowUpSuggestion[]>> {
  const env = options.env || runtimeEnv;
  const now = options.now || Date.now;
  const startedAt = now();
  const id = requestId();
  const prompt = PROMPT_REGISTRY["follow-up.suggest"];
  const config = getAiModelConfiguration(env);
  let provider: string = config.provider;
  let model = "none";
  let attempts = 0;
  let usage = {};
  try {
    if (!isAiCapabilityEnabled("follow-up.suggest", env)) throw new AiGatewayError("AI_FEATURE_DISABLED", { httpStatus: 503 });
    if (!input.actorId) throw new AiGatewayError("AI_INPUT_INVALID", { httpStatus: 400 });
    const approved = validateVisitSummaryOutput(input.approvedSummary);
    const protectedInput = protectPayload({ approvedSummary: approved, sourceDate: input.sourceDate });
    const protectedValue = protectedInput.value as { approvedSummary: unknown; sourceDate: string };
    const safeApproved = validateVisitSummaryOutput(protectedValue.approvedSummary);
    (options.rateLimiter || sharedRateLimiter).check(`${input.actorId}:follow-up.suggest`, Math.min(config.requestsPerMinute, 8), now());
    const adapter = options.adapter || new GeminiProviderAdapter(env);
    provider = adapter.id;
    const result = await adapter.generateStructured({
      systemPrompt: prompt.system,
      retryPrompt: prompt.retrySuffix,
      userPayload: buildFollowUpSuggestionUserPayload(safeApproved, protectedValue.sourceDate),
      responseSchema: FOLLOW_UP_SUGGESTION_RESPONSE_SCHEMA,
      models: config.models,
      timeoutMs: config.requestTimeoutMs,
      totalTimeoutMs: config.totalTimeoutMs,
      maxSafeRetries: config.maxSafeRetries,
      validateOutput: (value) => validateFollowUpSuggestionOutput(value, safeApproved, protectedValue.sourceDate),
    });
    model = result.model; attempts = result.attempts; usage = result.usage;
    const telemetry: GatewayTelemetry = {
      requestId: id, capability: "follow-up.suggest", provider, model,
      promptVersion: prompt.version, schemaVersion: FOLLOW_UP_SUGGESTION_SCHEMA_VERSION,
      outcome: "success", latencyMs: Math.max(0, now() - startedAt), attempts, usage,
    };
    auditLog(telemetry);
    return { output: result.output, telemetry, redaction: protectedInput.report };
